...
OUTPUT_DIR = 'staging_demo' # Images download location
...
if __name__ == '__main__':   
    ...
    #    for i in range(len)
    #    args = ((qryTermsList[termIndx],termIndx) for termIndx in qryTermsList)
        executor.map(getSETermImgs,qryTermsList) """


    while(termIndx<=len(qryTermsList)-1):
        processes = []
        for _ in range(4):
            p = multiprocessing.Process(target=getSETermImgs, args=[qryTermsList[termIndx]])
            p.start()
            processes.append(p)
            termIndx += 1
